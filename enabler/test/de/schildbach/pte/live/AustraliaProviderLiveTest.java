 ...
 * a lot of public transport available). If they are unable to find a route for a specific mode of transport, then
 * you should further investigate to see if the data is out of date or not in Navitia.
 *
 * Note that by default, only Melbourne is tested comprehensively to prevent running into API limits ({@see #RUN_EXPENSIVE_TESTS}).
 */
public class AustraliaProviderLiveTest extends AbstractNavitiaProviderLiveTest {

    /**
     * If enabled, the entire set of tests will run, resulting in over 30 API calls to Navitia.
     * Given this test may or may not be run under, e.g. continuous integration, or run frequently while working on
     * the Australian API, it could end up using up your API limit unneccesarily. Thus, the default value is to only
     * perform a proper test of Melbourne, and the rest of the coverage is disabled until this flag is true.
     */
    ...
    /**
     * Ensures that each of the suburban/rural trains, trams, and buses are represented in the journey planning
     * and location suggestion API.
     * Based on travelling around the Camberwell area: http://www.openstreetmap.org/#map=15/-37.8195/145.0586&layers=T
     */
    @Test
    public void melbourne() throws IOException {
        ...
        assertJourneyExists(AustraliaProvider.NETWORK_PTV, new String[] {"Lilydale", "Belgrave", "Alamein"}, suburbanTrainStation, ruralTrainStation);
    ...
    public void adelaideRail() throws IOException {
        ...
        assertJourneyExists(AustraliaProvider.NETWORK_SA, new String[]{"GRNG", "BEL", "OUTHA"}, railwayStation, railwayStation2);
    }
    ...
    // Although Navitia has a GTFS feed for ACT, Darwin, and Alice Springs, they were out of date at time of writing.
    @Test @Ignore public void act() {}
    @Test @Ignore public void darwin() {}
    @Test @Ignore public void aliceSprings() {}

    /**
     * Suggests locations similar to {@param locationName}, but then ensures that one matches exactly and then returns it.
     * Try not to use an ambiguous name such as "Central Station", because it may exist in several datasets on Navitia.
     */
    private Location assertAndGetLocation(String locationName) throws IOException {
        ...
        throw new AssertionError("suggestLocations() did not find \"" + locationName + "\". Options were: " + nonMatching);
    }

    /**
     * @see #assertJourneyExists(String, String[], Location, Location)
     */
    private void assertJourneyExists(String network, String eligibleLine, Location from, Location to) throws IOException {
    ...
    private void assertJourneyExists(String network, String[] eligibleLines, Location from, Location to) throws IOException {
        ...
        for (Trip trip : trips.trips) {
            ...
            for (Trip.Leg leg : trip.legs) {
                ...
                sb.append('[').append(leg.arrival.name).append(']').append(via).append('[').append(leg.departure.name).append(']').append("  ...  ");
            }
        }

        fail("No public trip found between [" + from.name + "] and [" + to.name + "] using appropriate line. Found trips:" + sb);
    }
}
