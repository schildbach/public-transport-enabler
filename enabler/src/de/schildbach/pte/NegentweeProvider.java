...
public class NegentweeProvider extends AbstractNetworkProvider {
    ...
    private static class TripsContext implements QueryTripsContext {
        ...
        TripsContext(HttpUrl url, @Nullable String earlier, @Nullable String later, Location from, @Nullable Location via, Location to) {
    ...
    private HttpUrl buildApiUrl(String action, List<QueryParameter> queries) {
        HttpUrl.Builder url = HttpUrl.parse(API_BASE).newBuilder()
                .addPathSegments(action)
                .addQueryParameter("lang", this.language.toString());
    ...
    private Trip tripFromJSONObject(JSONObject trip, @Nullable Location from, @Nullable Location to, @Nullable Map<String, JSONObject> disturbances) throws JSONException {
            ...
            switch (leg.getString("type").toLowerCase()) {
                case "scheduled":
                    ...
                    foundLegs.add(new Trip.Public(
                            new Line(
                                    leg.getString("service"),
                                    (operator != null) ? operator.getString("name") : null,
                                    lineProduct,
                                    lineName.toString(),
                                    leg.optString("service"),
                                    Standard.STYLES.get(lineProduct),
                                    null,
                                    null
                            ),
                            new Location(
                                    ...
                                    null,
                                    null,
                                    leg.getString("destination")
                            ),
                            firstStop,
                            lastStop,
                            foundStops,
                            foundPoints,
                            legMessage.length() > 0 ? legMessage.toString() : null
                    ));
                    break;
                case "continuous":
                    // Get leg time from trip or previous leg
                    Date legDeparture = (i == 0) ? tripDeparture : foundLegs.getLast().getArrivalTime();
                    Date legArrival = ParserUtils.addMinutes(legDeparture, ParserUtils.parseMinutesFromTimeString(leg.getString("duration")));

                    foundLegs.add(new Trip.Individual(
    ...
    private Stop stopFromJSONObject(JSONObject stop) throws JSONException {
        ...
        return new Stop(
                locationFromJSONObject(stop.getJSONObject("location")),
                dateFromJSONObject(stop, "arrival"),
                dateFromJSONObject(stop, "realtimeArrival"),
                plannedPlatform,
                changedPlatform,
                false,
                dateFromJSONObject(stop, "departure"),
                dateFromJSONObject(stop, "realtimeDeparture"),
                plannedPlatform,
                changedPlatform,
                false
        );
    ...
    private Fare fareFromJSONObject(JSONObject fareLeg) throws JSONException {
        ...
        return new Fare(
                fareLeg.getString("operatorString"),
                Fare.Type.ADULT,
    ...
    private Departure departureFromJSONObject(JSONObject departure) throws JSONException {
        ...
        return new Departure(
                ...
                new Line(
                        null,
                        departure.getString("operatorName"),
                        lineProduct,
                ...
                new Location(
                        ...
                        null,
                        null,
                        departure.getString("destinationName")
                ),
                null,
    ...
    private Location locationFromJSONObject(JSONObject location, boolean addTypePrefix) throws JSONException {
        ...
        return new Location(
                locationTypeFromTypeString(locationType),
                location.getString("id"),
                locationPoint.lat,
                locationPoint.lon,
                !(place == null) ? place.getString("name") : null,
                locationName,
                null
        );
    }
    ...
    private QueryTripsResult ambiguousQueryTrips(Location from, @Nullable Location via, Location to) throws IOException {
    ...
    private QueryTripsResult queryTrips(HttpUrl url, Location from, @Nullable Location via, Location to) throws IOException {
    ...
    public Set<Product> defaultProducts() {
        return EnumSet.of(Product.HIGH_SPEED_TRAIN, Product.REGIONAL_TRAIN, Product.SUBURBAN_TRAIN,
                Product.SUBWAY, Product.TRAM, Product.BUS, Product.FERRY);
    }
        ...
        try {
            ...
            for (int t = 0; t < tabs.length(); t++) {
                ...
                for (int l = 0; l < locations.length(); l++) {
                    ...
                    ArrayList<Departure> departuresResult = new ArrayList<>();
                    ArrayList<LineDestination> lineDestinationResult = new ArrayList<>();
                    ...
                    queryDeparturesResult.stationDepartures.add(new StationDepartures(
                            locationFromJSONObject(location),
                            departuresResult,
                            lineDestinationResult
                    ));
                }
            }
    ...
    public SuggestLocationsResult suggestLocations(CharSequence constraint) throws IOException {
        HttpUrl url = buildApiUrl("locations", Arrays.asList(
                new QueryParameter("q", constraint.toString())
        ));
        final CharSequence page;
        try {
            page = httpClient.get(url);
    ...
    public QueryTripsResult queryTrips(Location from, @Nullable Location via, Location to, Date date, boolean dep, @Nullable Set<Product> products, @Nullable Optimize optimize, @Nullable WalkSpeed walkSpeed, @Nullable Accessibility accessibility, @Nullable Set<Option> options) throws IOException {
        ...
        // Default query options
        ArrayList<QueryParameter> queryParameters = new ArrayList<>(Arrays.asList(
                new QueryParameter("from", from.id),
                new QueryParameter("to", to.id),
                ...
                new QueryParameter("sequence", "1"),
                new QueryParameter("realtime", "true"),
                new QueryParameter("before", "1"),
                new QueryParameter("after", "5")
        ));
        ...
        if (walkSpeed != null && walkSpeed == WalkSpeed.SLOW)
        {
            queryParameters.add(new QueryParameter(
                    "interchangeTime",
                    InterchangeTime.EXTRA.toString()
            ));
        } else {
            queryParameters.add(new QueryParameter(
                    "interchangeTime",
                    InterchangeTime.STANDARD.toString()
            ));
        }
        ...
        queryParameters.add(new QueryParameter(
                "byBus",
                String.valueOf(products.contains(Product.BUS))
        ));
        queryParameters.add(new QueryParameter(
                "byTrain",
                String.valueOf(products.contains(Product.HIGH_SPEED_TRAIN) || products.contains(Product.REGIONAL_TRAIN) || products.contains(Product.SUBURBAN_TRAIN))
        ));
        queryParameters.add(new QueryParameter(
                "bySubway",
                String.valueOf(products.contains(Product.SUBWAY))
        ));
        queryParameters.add(new QueryParameter(
                "byTram",
        ...
        queryParameters.add(new QueryParameter(
                "byFerry",
                String.valueOf(products.contains(Product.FERRY))
        ));
