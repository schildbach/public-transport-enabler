...
public abstract class AbstractNavitiaProvider extends AbstractNetworkProvider
{
	...
	private Line parseLine(final JSONObject jsonLine) throws IOException
	{
		try
		{
			...
			String network = null;
			if (jsonLine.has("network")) network = Strings.emptyToNull(jsonLine.getJSONObject("network").optString("name"));
			...
			throws IOException
	{
		...
		try
		{
			...
			for (int i = 0; i < departures.length(); ++i)
			{
				...
				if (stationDepartures.lines != null && !stationDepartures.lines.contains(lineDestination))
