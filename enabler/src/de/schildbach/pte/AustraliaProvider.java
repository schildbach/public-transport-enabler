...
public class AustraliaProvider extends AbstractNavitiaProvider {
    ...
    static {
        ...
        STYLES.put(NETWORK_PTV + "|SGlen Waverly", new Style(Style.Shape.RECT, Style.parseColor("#094B8D"), Style.WHITE));
        STYLES.put(NETWORK_PTV + "|SSunbury", new Style(Style.Shape.RECT, Style.parseColor("#FFB531"), Style.BLACK));
        STYLES.put(NETWORK_PTV + "|SCraigieburn", new Style(Style.Shape.RECT, Style.parseColor("#FFB531"), Style.BLACK));
        STYLES.put(NETWORK_PTV + "|SUpfield", new Style(Style.Shape.RECT, Style.parseColor("#FFB531"), Style.BLACK));
        STYLES.put(NETWORK_PTV + "|SSouth Morang", new Style(Style.Shape.RECT, Style.parseColor("#E42B23"), Style.WHITE));
        STYLES.put(NETWORK_PTV + "|SHurstbridge", new Style(Style.Shape.RECT, Style.parseColor("#E42B23"), Style.WHITE));
        ...
        STYLES.put(NETWORK_PTV + "|SWilliamstown", new Style(Style.Shape.RECT, Style.parseColor("#149943"), Style.WHITE));

        // Truncated version of "Sandringham". Not sure if this is a bug/limitation in either Navitia or the GTFS feed from PTV.
        STYLES.put(NETWORK_PTV + "|SSandringha", new Style(Style.Shape.RECT, Style.parseColor("#FC7EBB"), Style.BLACK));

        // Difficult to test this, because the line is open only for select periods of the year (e.g. for the Melbourne Show
        // in September) and at time of writing, the GTFS feed did not have data up until then.
        STYLES.put(NETWORK_PTV + "|SFlemington Racecourse", new Style(Style.Shape.RECT, Style.parseColor("#9A9B9F"), Style.BLACK));
        ...
        // The GTFS feed seems to combine 3 + 3a like this, but for completeness we include 3 and 3a separately too.
        ...
        // NOTE: This is a work around for poor GTFS data. We should instead say "All REGIONAL_TRAINs are purple",
        // but the GTFS feed from Navitia instead returns rural trains as SUBURBAN_TRAINs. Given we have already
        // provided colours for all suburban trains above, this more general statement about suburban trains
        // results in all regional trains being coloured purple, as intented.
        ...
        // At any rate, the EFA provider for Sydney is likely going to be better than this Navitia provider anyway.
        ...
        // These are already colourised correctly by the GTFS data given to Navitia.
        // The styles do not include "display_informations" with a proper "code" though, so the names are not displayed.
        // But for reference, the maps are available at http://www.transperth.wa.gov.au/Journey-Planner/Network-Maps.

        // Tasmania bus networks.
        // Somewhat colourised in Navitia (e.g. Launceston has green buses), but it is incorrect (e.g. Launceston should
        // have all sorts of different coloured buses).
        // Maps are available at https://www.metrotas.com.au/timetables/.
    }
